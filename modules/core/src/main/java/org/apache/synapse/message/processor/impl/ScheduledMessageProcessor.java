 ...
 * Mainly responsible for handling life cycle states of the message processors. Some of
...
public abstract class ScheduledMessageProcessor extends AbstractMessageProcessor implements TaskManagerObserver{
    ...
    public boolean stop() {
        ...
        if (nTaskManager != null && nTaskManager.isInitialized()) {
            for (int i = 0; i < memberCount; i++) {
				...
				/*
				 * Gracefully shutdown.This value should be given in the format --> taskname::taskgroup.
    ...
    public void destroy() {
		 ...
		 * the scheduler which will cause to shutdown the job
		 */
        try {
            stop();
        }
    ...
    public boolean deactivate() {
        if (nTaskManager != null && nTaskManager.isInitialized()) {
            try {
				...
				/*
				 * This is to remove the consumer from the queue & close the connection
				 * with the JMS Provider/message store.
				 */
    ...
    public boolean activate() {
		...
		// Checking whether it is already deactivated. If it is deactivated only we can activate again.
